...
import org.jared.dungeoncrawler.v1_13_R2.PacketUtil;
...
public class MapUtil implements IMapUtil
{
    ...
    private void sendItemFrame(Player player, int itemFrameID, ItemStack item)
    {
        try
        {
            ...
            PacketUtil.sendPacket(packetPlayOutEntityMetadata, player);
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    public int spawnFakeItemFrame(Collection<? extends Player> players, Location location, BlockFace facingDirection)
    {
        ...
        {
            PacketUtil.sendPacket(spawnEntityPacket, player);
        }
